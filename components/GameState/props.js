...
export const mlbGameStateProps = game => {
  ...
  const inBetween =
    inningState && inningState !== 'Middle' && inningState !== 'End'
  ...
  return {
    ...
    time: isDelayed
      ? `${time} - ${game.status.detailedState} ${
          game.status.reason !== undefined ? `(${game.status.reason})` : ''
        }`
      : time,
    periods: 9,
    currentTime:
      inBetween &&
      `${game.linescore.outs} ${game.linescore.outs === 1 ? 'out' : 'outs'}`,
    currentPeriod:
      inGame &&
      `${inningState === 'End' ? inningState.toUpperCase() : inningState} ${
